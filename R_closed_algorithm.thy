...
lemma R_closed_algo_RL:"r2=r \<Longrightarrow> R_closed_algo r r2 f = r \<Longrightarrow> (s,t)\<in>set r \<Longrightarrow> f\<noteq>[] \<Longrightarrow> equiv (allstates f) (set r)  \<Longrightarrow> (get_R_states_set r)= (get_NFTS_states_set f)\<Longrightarrow>
...
lemma R_closed_algo_cor:
 assumes "equiv (allstates f) (set r)"
          ...
          "r=r2"
          "f\<noteq>[]"
  shows " R_closed_algo r r2 f = r \<Longrightarrow> BIS_RL r r2 f"
  ...
  using R_closed_algo_RL assms apply blast
  apply (induct r)
  apply auto
...
theorem algorithm_cor:
  assumes "equiv (allstates f) (set r) "
          "(get_R_states_set r)= (get_NFTS_states_set f)"
          "f\<noteq>[]"
  shows "r=algo_induct r1 f \<Longrightarrow> BIS_RL r r f"
  using assms
